...
from asap_data import TextLoader
from model import LSTMClassifier
from logisticRegModel import LogisticRegression
from maLSTM import maLSTMClassifier


# python3 asap_train.py --data_dir /Users/yajurtomar/psu/NLPResearch/ASGA/data/asap-sas/train.tsv --test_dir /Users/yajurtomar/psu/NLPResearch/ASGA/data/asap-sas/test --batch_size 1 --hidden_dim 16 --char_dim 100 --num_epochs 50
...
def evaluate_validation_set(model, devset, x_to_ix, y_to_ix, criterion):
    ...
    y_pred = list()
    total_loss = 0
    for batch, targets, lengths, raw_data in utils.create_dataset(devset, x_to_ix, y_to_ix,
                                                                                            batch_size=1):
        batch, targets, lengths = utils.sort_batch(batch, targets, lengths)
        pred, loss = apply(model, criterion, batch, targets, lengths)
        pred_idx = torch.max(pred, 1)[1]
