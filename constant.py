...
# TRAIN_FILE_PATH = ['./data/Istudio/debug.csv']
# TEST_FILE_PATH = ['./data/Istudio/debug.csv']

TRAIN_FILE_PATH = ['./data/Istudio/traindev_set.csv']
TEST_FILE_PATH = ['./data/Istudio/test_set.csv']
...
#                         ],
                 'q2_b': ["You could give her a test on the notes and the more she answers correctly, then the better she is at music.",
                        "I would play a certain amount of notes and find the average amount Carla got correct. If the mean was greater than 75 percent, I would say she has a good ear for music.",
                        "Conduct an experiment in which Carla is placed in an experimental group and some other students are placed in a control group. Play the notes and measure the amount of times Carla picks the right note and compare this to the number of times the control subjects picked the right note. This comparison could be done by constructing bar graphs with yes or no for whether or not Carla picked the right note.",
                        "I would give each student at least ten trials and figure out the median notes indentified in the classroom students that found themselves above the IQR would be considered \"good ear\".",
                        ],
...
#                         ]
                 'q4_b': ["You could compare the average length of each of their catches and their fishing habits. The one with more and larger fish is the better fisherman.",
                        "The amount of fish each brother caught would begin to explain who is the better fisherman. If one was a standout fisherman then it is obvious. If they are similar then the average fish size could tell us who is better.",
                        "Using data from the fishing trip, I would plot the data using a scatterplot of the lengths of the fish that each brother caught and then run a significance test to see if it is a significant difference.",
                        "I would record the lengths and quantity of fish each brother caught in a list and then use that data to conduct a matched pairs test to see who is in fact the superior fisherman.",
                        ]
}
